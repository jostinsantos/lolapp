...
void VideoOutput::Render() {
  if (texture_id_) {
    ...
    if (pixel_buffer_ != nullptr) {
      ...
      mpv_render_context_render(render_context_, params);
      // mpv's "rgb0" format leaves the fourth byte undefined. Flutter uploads
      // pixel-buffer textures as RGBA, so make every pixel fully opaque before
      // publishing this software-rendered frame.
      const auto pixel_count = static_cast<size_t>(size[0]) * size[1];
