...
class _MoviePageState extends State<MoviePage> {
  ...
  Future<void> _cargarContenido({bool reset = false}) async {
    ...
    try {
      final generoParam = _generoSeleccionado == 'Recientes'
          ? 'recientes'
          : _generoSeleccionado;

      final uri = Uri.parse(
        '$_kBaseUrl/genero.php?tipo=movie&genero=${Uri.encodeComponent(generoParam)}&page=$_page',
      );
  ...
  void _abrirContenido(Map<String, dynamic> item) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => PageContenido(idcontenido: idInt)),
    );
  }
