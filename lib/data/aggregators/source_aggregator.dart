...
class FuentesConfig {
  ...
  List<FuenteId> get fuentesActivas {
    ...
    // CineSrc = solo embeds WebView → al final (fallback)
...
class MainFuentes {
  ...
  Stream<FuenteEvent> fetchProgressive({
    ...
    // CineSrc = embeds WebView → al final y solo si no hay otros servidores
    final prioritarias = activas.where((f) => f != FuenteId.cinesrc).toList();
    final incluirCinesrc = activas.contains(FuenteId.cinesrc);

    final seenUrls = <String>{};
    var servidoresOtrasFuentes = 0;

    Future<void> runFuente(FuenteId fuente) async {
      ...
      try {
        await for (final raw in _scrapeFuente(
          ...
          if (fuente == FuenteId.cinesrc) {
            map['es_cinesrc'] = true;
            map['webview_only'] = true;
            map['prioridad'] = 999;
          }
          ...
          // CineSrc: no gastar WebView del verificador (ya son embeds)
          final debeVerificar = verificar &&
              context != null &&
              context.mounted &&
              verifiedAttempts < _kMaxVerifyPerFuente &&
              fuente != FuenteId.cinesrc;
          ...
          if (debeVerificar) {
            ...
            if (m3u8 != null && m3u8.isNotEmpty) {
              ...
              map['verificado'] = true;
              count++;
              if (fuente != FuenteId.cinesrc) servidoresOtrasFuentes++;
        ...
        if (!controller.isClosed) {
          controller.add(FuenteEvent(fuente: fuente, isDone: true));
        }
      } catch (e) {
        if (!controller.isClosed) {
          controller.add(FuenteEvent(
    ...
    // Orquestación: prioritarias en paralelo → luego CineSrc condicional
    () async {
      try {
        if (prioritarias.isNotEmpty) {
          await Future.wait(prioritarias.map(runFuente));
        }

        // Solo CineSrc si no hubo ningún servidor de otras fuentes
        if (incluirCinesrc && !controller.isClosed) {
          if (servidoresOtrasFuentes == 0) {
            debugPrint(
              'MainFuentes: sin servidores de otras fuentes → fallback CineSrc',
            );
            await runFuente(FuenteId.cinesrc);
          } else {
            debugPrint(
              'MainFuentes: $servidoresOtrasFuentes servidores OK → omite CineSrc',
            );
            // Marca done sin scrapear para que el modal no quede esperando
            if (!controller.isClosed) {
              controller.add(const FuenteEvent(
                fuente: FuenteId.cinesrc,
                isDone: true,
              ));
            }
